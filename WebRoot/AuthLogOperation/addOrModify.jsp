...
<% 
	...
	// 对象实例 
	AuthLogOperationObj domainInstance = new AuthLogOperationObj(); 
	// 该功能基本路径 
	...
	{ 
		domainInstance = (AuthLogOperationObj) domainInstanceObj; 
	} 
...
<html> 
	<head> 
		...
		if(!checkNull("useraccount","<%=domainInstance.getPropertyCnName("useraccount")%>")) return false; 
		if(!checkNull("username","<%=domainInstance.getPropertyCnName("username")%>")) return false; 
		if(!checkNull("userdepartmentcode","<%=domainInstance.getPropertyCnName("userdepartmentcode")%>")) return false; 
		if(!checkNull("module_name","<%=domainInstance.getPropertyCnName("module_name")%>")) return false; 
		if(!checkNull("module_method","<%=domainInstance.getPropertyCnName("module_method")%>")) return false; 
		if(!checkNull("full_info","<%=domainInstance.getPropertyCnName("full_info")%>")) return false; 
		if(!checkNull("timestamp","<%=domainInstance.getPropertyCnName("timestamp")%>")) return false; 
		...
		<form name="addOrModifyForm" id="addOrModifyForm" action="<%=contextPath%>/<%=basePath%>/Servlet?method=addOrModify4this" method="post"> 
			...
			<table width="700" align="center" class="detail_table detail_table-bordered detail_table-striped"> 
				...
				<tr> 
					...
					<td> 
						<%=DictionaryUtil.getSelectHtml("部门字典", "userdepartmentcode", StringUtil.getNotEmptyStr(domainInstance.getUserdepartmentcode(), ""))%> 
						<font color="red">*</font> 
					</td> 
				</tr> 
				<tr> 
				...
				<tr> 
					...
					<td> 
						<input name="module_method" type="text" id="module_method" value="<%=StringUtil.getNotEmptyStr(domainInstance.getModule_method(),"")%>" size="20"  > 
						<font color="red">*</font> 
					</td> 
				</tr> 
				<tr> 
					...
					<td> 
						<input name="full_info" type="text" id="full_info" value="<%=StringUtil.getNotEmptyStr(domainInstance.getFull_info(),"")%>" size="20"  > 
						<font color="red">*</font> 
					</td> 
				</tr> 
				<tr> 
