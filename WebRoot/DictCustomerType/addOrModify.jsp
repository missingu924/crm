...
<%@page import="com.crm.obj.DictCustomerTypeObj"%> 
<!-- 基本信息 -->  
<% 
	// 上下文路径 
	...
	// 对象实例 
	DictCustomerTypeObj domainInstance = new DictCustomerTypeObj(); 
	// 该功能基本路径 
	...
	{ 
		domainInstance = (DictCustomerTypeObj) domainInstanceObj; 
	} 
 
	// 是否是修改 
	boolean isModify = domainInstance.getKeyValue() > 0; 
	// 唯一性检查用的字段 
	String keyCol = "customer_type_code"; 
%> 
<html> 
	<head> 
		<base target="_self" /> 
		...
		if(!checkNull("customer_type_name","<%=domainInstance.getPropertyCnName("customer_type_name")%>")) return false; 
		...
		<form name="addOrModifyForm" id="addOrModifyForm" action="<%=contextPath%>/<%=basePath%>/Servlet?method=addOrModify4this" method="post"> 
			...
			<table width="700" align="center" class="detail_table detail_table-bordered detail_table-striped"> 
				...
				<tr> 
					...
					<td> 
						<input name="customer_type_code" type="text" id="customer_type_code" value="<%=StringUtil.getNotEmptyStr(domainInstance.getCustomer_type_code(),"")%>" size="20"  <%=isModify?"readOnly":""%>> 
						<font color="red">*</font> 
				...
				<tr> 
					<td> 
						<%=domainInstance.getPropertyCnName("customer_type_name") %>: 
					</td> 
					<td> 
						<input name="customer_type_name" type="text" id="customer_type_name" value="<%=StringUtil.getNotEmptyStr(domainInstance.getCustomer_type_name(),"")%>" size="20"  > 
						<font color="red">*</font> 
					</td> 
				</tr> 
			</table> 
