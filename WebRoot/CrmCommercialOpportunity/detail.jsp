	...
	<body>
		...
		<table width="720" align="center" class="top_tools_table">
			<tr>
				<td>
					...
					<%if(user.hasFunction("商机-增加")){ %><input id="copyButton" type="button" class="button button_copy" value="复制" onClick="winOpen('<%=contextPath%>/<%=basePath%>/Servlet?method=copy4this&<%=domainInstance.findKeyColumnName()%>=<%=domainInstance.getKeyValue()%>')"><%} %>
		...
		<table width="720" align="center" class="detail_table detail_table-bordered">
			...
			<tr>
				...
				<td><%=DictionaryUtil.getDictValueByDictKey("账号字典", StringUtil.getNotEmptyStr(domainInstance.getManagement_account(), ""))%>				</td>
				<td>&nbsp;</td>
				<td>&nbsp;</td>
			</tr>
			<tr>
				<td><%=domainInstance.getPropertyCnName("management_type_code")%>: </td>
				<td><%=DictionaryUtil.getDictValueByDictKey("经营类型字典", StringUtil.getNotEmptyStr(domainInstance.getManagement_type_code(), ""))%> </td>
			...
			<tr>
				...
				<td><%=TimeUtil.date2str(domainInstance.getEstimate_sign_time(), "yyyy-MM-dd")%></td>
		    </tr>
			<tr>
				<td>
					<%=domainInstance.getPropertyCnName("source")%>:				</td>
				<td>
					<%=StringUtil.getNotEmptyStr(domainInstance.getSource(), "")%>				</td>
			    <td>&nbsp;</td>
			    <td>&nbsp;</td>
		    </tr>
			<tr>
				<td><%=domainInstance.getPropertyCnName("customer_request")%>:				</td>
				<td><%=StringUtil.getNotEmptyStr(domainInstance.getCustomer_request(), "").replaceAll("\n","<br>")%>				</td>
			    <td>&nbsp;</td>
			    <td>&nbsp;</td>
		    </tr>
			<tr>
				<td><%=domainInstance.getPropertyCnName("next_step")%>:				</td>
				<td><%=StringUtil.getNotEmptyStr(domainInstance.getNext_step(), "").replaceAll("\n","<br>")%>				</td>
			    <td>&nbsp;</td>
			    <td>&nbsp;</td>
		    </tr>
			<tr>
