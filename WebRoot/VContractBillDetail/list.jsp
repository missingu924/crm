			...
			<table class="search_table" align="center" width="98%"> 
				<tr> 
					<td align="left"> 
						...
						<%=DictionaryUtil.getInputHtml("经营类型字典", "management_type_code", StringUtil.getNotEmptyStr(domainInstance.getManagement_type_code(), ""))%></td>  
				    <td align="left"><%=domainInstance.getPropertyCnName("customer_id") %> <%=DictionaryUtil.getInputHtml("客户字典", "customer_id", StringUtil.getNotEmptyStr(domainInstance.getCustomer_id(), ""),user.hasRole(SystemConstant.ROLE_ADMIN) ? "" : "id in(select id from crm_customer where (customer_manager_account like \\'%," + user.getAccount()
								+ ",%\\' or service_engineer_account like \\'%," + user.getAccount() + ",%\\'))")%></td>
				    <td align="left"><%=domainInstance.getPropertyCnName("contract_subject") %> <%=DictionaryUtil.getInputHtml("合同主体字典", "contract_subject", StringUtil.getNotEmptyStr(domainInstance.getContract_subject(), ""))%></td>
				    <td align="left"><%=domainInstance.getPropertyCnName("contract_id") %> <%=DictionaryUtil.getInputHtml("合同字典", "contract_id", StringUtil.getNotEmptyStr(domainInstance.getContract_id(), ""),user.hasRole(SystemConstant.ROLE_ADMIN) ? "" : "customer_id in(select id from crm_customer where (customer_manager_account like \\'%," + user.getAccount()
								+ ",%\\' or service_engineer_account like \\'%," + user.getAccount() + ",%\\'))")%></td>
				    <td align="left"><%=domainInstance.getPropertyCnName("bill_receipt") %> <%=DictionaryUtil.getInputHtml("是否字典", "bill_receipt", StringUtil.getNotEmptyStr(domainInstance.getBill_receipt(), ""))%></td>
				    <td align="left"><%=domainInstance.getPropertyCnName("is_finished") %> <%=DictionaryUtil.getInputHtml("是否字典", "is_finished", StringUtil.getNotEmptyStr(domainInstance.getIs_finished(), ""))%></td>
				    <td rowspan="2" align="left"><input name="searchButton" type="button" class="button button_search" value="查询" onClick="toPage(1)"></td>
				</tr>
				<tr>
				  <td colspan="2" align="left"><%=domainInstance.getPropertyCnName("contract_sign_time") %>
...
-
<input name="contract_sign_time_end" type="text" id="contract_sign_time_end" value="<%=StringUtil.getNotEmptyStr(domainSearchCondition.getContract_sign_time_end())%>" size="11" onFocus="WdatePicker({isShowClear:false,readOnly:false,highLineWeekDay:true,dateFmt:'yyyyMMdd'})"></td>
				  <td colspan="2" align="left"><%=domainInstance.getPropertyCnName("bill_date") %>
...
-
  <input name="bill_date_end" type="text" id="bill_date_end" value="<%=StringUtil.getNotEmptyStr(domainSearchCondition.getBill_date_end())%>" size="11" onFocus="WdatePicker({isShowClear:false,readOnly:false,highLineWeekDay:true,dateFmt:'yyyyMMdd'})"></td>
				  <td colspan="2" align="left"><%=domainInstance.getPropertyCnName("gather_date") %>
...
-
  <input name="gather_date_end" type="text" id="gather_date_end" value="<%=StringUtil.getNotEmptyStr(domainSearchCondition.getGather_date_end())%>" size="11" onFocus="WdatePicker({isShowClear:false,readOnly:false,highLineWeekDay:true,dateFmt:'yyyyMMdd'})"></td>
			  </tr> 
			</table> 
