	...
	<body> 
		<form name="pageForm" id="pageForm" method="post" action="<%=request.getContextPath()%>/<%=basePath%>/Servlet?method=list4this"> 
			...
			<table class="search_table" align="center" width="98%">
				<tr>
				    ...
				    <td><%=DictionaryUtil.getInputHtml("客户字典", "customer_id", StringUtil.getNotEmptyStr(domainInstance.getCustomer_id(), ""),user.hasFunction("无限制查询") ? "" : "id in(select id from crm_customer where (customer_manager_account like \\'%," + user.getAccount()
								+ ",%\\' or service_engineer_account like \\'%," + user.getAccount() + ",%\\'))",40)%></td>
			...
			<table id="search_condition_table" class="search_condition_table" align="center" width="98%" style="display:<%=domainSearchCondition.isShowSearchConditionTable()?"":"none" %>">
				...
				<tr>
				  <td><%=domainInstance.getPropertyCnName("commercial_oppotunity_id") %></td>
			      <td><%=DictionaryUtil.getInputHtml("商机字典-选择用", "commercial_oppotunity_id", StringUtil.getNotEmptyStr(domainInstance.getCommercial_oppotunity_id(), ""),user.hasFunction("无限制查询") ? "" : "customer_id in(select id from crm_customer where (customer_manager_account like \\'%," + user.getAccount()
								+ ",%\\' or service_engineer_account like \\'%," + user.getAccount() + ",%\\'))")%></td>
			      <td><%=domainInstance.getPropertyCnName("activity_type") %></td>
			      <td><%=DictionaryUtil.getInputHtml("经营活动类型字典", "activity_type", StringUtil.getNotEmptyStr(domainInstance.getActivity_type(), ""))%></td>
			  </tr>
				<tr>
