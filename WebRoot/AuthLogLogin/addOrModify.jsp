...
<% 
	...
	// 对象实例 
	AuthLogLoginObj domainInstance = new AuthLogLoginObj(); 
	// 该功能基本路径 
	...
	{ 
		domainInstance = (AuthLogLoginObj) domainInstanceObj; 
	} 
...
<html> 
	<head> 
		...
		if(!checkNull("userdistrict","<%=domainInstance.getPropertyCnName("userdistrict")%>")) return false; 
		if(!checkNull("userdepartmentid","<%=domainInstance.getPropertyCnName("userdepartmentid")%>")) return false; 
		if(!checkNull("userdepartmentname","<%=domainInstance.getPropertyCnName("userdepartmentname")%>")) return false; 
		...
		<form name="addOrModifyForm" id="addOrModifyForm" action="<%=contextPath%>/<%=basePath%>/Servlet?method=addOrModify4this" method="post"> 
			...
			<table width="700" align="center" class="detail_table detail_table-bordered detail_table-striped"> 
				...
				<tr> 
					...
					<td> 
						<input name="userdepartmentcode" type="text" id="userdepartmentcode" value="<%=StringUtil.getNotEmptyStr(domainInstance.getUserdepartmentcode(),"")%>" size="20"  > 
						<font color="red">*</font> 
					</td> 
				</tr> 
				<tr> 
