	...
	<body>
		<form name="pageForm" id="pageForm" method="post" action="<%=request.getContextPath()%>/<%=basePath%>/Servlet?method=list4this">
			...
			<table class="search_table" align="center" width="98%">
				<tr>
					<td><%=domainInstance.getPropertyCnName("id")%></td>
					<td><%=DictionaryUtil.getInputHtml("合同字典-选择用", "id", StringUtil.getNotEmptyStr(domainInstance.getId(), ""), user.hasFunction("无限制查询") ? "" : "customer_id in(select id from crm_customer where (customer_manager_account like \\'%," + user.getAccount()
